...
__Detailed Findings:__
* __Baseline Comparison:__ We introduce a baseline method, OnlyPrompt, which uses a straightforward PEFT approach without any complex query mechanisms. This method performs on par with or better than existing P-RFCL methods across a diverse set of benchmarks.
* __Parameter Efficiency:__ OnlyPrompt achieves these results while using significantly fewer parameters, highlighting the efficiency of simple PEFT techniques.
...
__Detailed Findings:__
* __Parameter Trade-off:__ The study finds that there is an optimal range for the number of tunable parameters that balances adaptation and forgetting. Too few parameters lead to insufficient adaptation, while too many parameters increase the risk of forgetting.
...
__Detailed Findings:__
* __Comparative Analysis:__ We compare first-task adaptation methods, such as Nearest Mean Classifier (NMC) and ADAM (Zhou et al., 2023), against full adaptation approaches like OnlyPrompt. The results show that full adaptation outperforms first-task adaptations across multiple benchmarks.
...
__Detailed Findings:__
* __Integration with PEFT:__ Our study explores the integration of EWC and SI with the OnlyPrompt baseline. We find that these regularization techniques can further reduce forgetting and improve overall performance.
